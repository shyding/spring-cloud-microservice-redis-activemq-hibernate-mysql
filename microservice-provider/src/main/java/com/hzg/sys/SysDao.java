...
public class SysDao {
    ...
    public String save(Object object, Class clazz, String redisKeyPerfix){
        ...
        if (dbObject != null) {
            try {
                redisTemplate.opsForValue().set(redisKeyPerfix + String.valueOf(clazz.getMethod("getId").invoke(object)), dbObject);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public String updateById(Integer id, Object object, Class clazz, String redisKeyPerfix){
        ...
        Object dbObject = sessionFactory.getCurrentSession().createCriteria(clazz).
                add(Example.create(object).excludeZeroes()).uniqueResult();
        if (dbObject != null) {
            redisTemplate.opsForValue().set(redisKeyPerfix + id, dbObject);
        }
    ...
    public List query(Integer id, Object object, Class clazz, String redisKeyPerfix){
        ...
        return sessionFactory.getCurrentSession().createCriteria(Company.class).add(Example.create(object).excludeZeroes()).list();
    }
    ...
    public Object queryById(Integer id, Object object, Class clazz, String redisKeyPerfix){
        ...
        if (dbObject == null) {
            ...
            if (dbObject != null) {
                valueOperation.set(redisKeyPerfix + id, dbObject);
            }
        }
